...
if (autoplayResumeTimeout) return;
startCertificateAutoplay();
});
...
loadChatbot(() => {
if (window.setAiChatOpen) {
window.setAiChatOpen(aiChatPanel?.hidden ?? true);
}
});
});
...
const scrollTop = window.scrollY || document.documentElement.scrollTop;
const triggerPoint = scrollTop + window.innerHeight * 0.6;
